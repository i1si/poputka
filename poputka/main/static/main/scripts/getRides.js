...
if (rideFrom && rideTo && rideDate && ridePersons) {
    fetch(baseURL + '/api/v1/rides/?from=' + rideFrom + '&to=' + rideTo + '&date=' + rideDate + '&persons=' + ridePersons)
    .then(res => {
        if (res.ok) {
            return res.json();
        } else {
...
function displayRides(rides) {
    if (rides.length) {
        const content = rides.map((list) => {
            ...
            let datetimeStr = getDatetimeStr(list['ride_datetime'])
            let rideURL = '<a href="' + baseURL + '/rides/' + list['id'] + '/" target="_blank" class="ride-box">'
            return '<div class="a-opac">' + rideURL + '<div class="ride-time">' + datetimeStr + '</div><div class="ride-info"><div class="ride-route">' + list['from_place'] + ' - ' + list['to_place'] + '</div><div class="ride-bottom"><div class="ride-user">' + driverAvatar + '<div>' +list['driver']['first_name'] + '<div class="ride-user-rating"><svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path fill="white" d="m354-247 126-76 126 77-33-144 111-96-146-13-58-136-58 135-146 13 111 97-33 143ZM233-80l65-281L80-550l288-25 112-265 112 265 288 25-218 189 65 281-247-149L233-80Zm247-350Z"/></svg><span class="ride-user-rating-text">' + list['driver']['rating'] + '/5</span></div></div></div><div class="ride-seats">Мест: ' + list['seats_count'] + '</div></div></div><div class="ride-price">' + list['price'] + ' ₽</div></a></div>'
        })
        ridesList.innerHTML = content.join("")
    } else {
        ridesList.innerHTML = 'Поездок нет'
    }
}